...
// Function used to save game to a pre-specified .txt file
void pauseSave(Table& t) {
    ...
    while(true) {
        ...
        if(ans == "Yes" || ans == "yes") {
            ofstream outFile("test.txt"); // Open file
...
Table* loadFromSave (CardFactory *cf) {
    ifstream inFile("test.txt"); // Open file
    if (inFile.is_open()) {
        ...
        if(playerTurn == 1) { //Player1 turn
            ...
            if(ans == "yes" || ans == "Yes") {
                ...
                while(gameTable->p1.hand.hand.empty() == false) {
                    int position = rand() % (gameTable->p1.hand.hand.size());
            ...
            if(gameTable->tArea.numCards() != 0) {
                for(auto itr = gameTable->tArea.cardTypes.begin(); itr != gameTable->tArea.cardTypes.end(); ++itr) {
                    while(true){
                        ...
                        if(ans == "yes" || ans == "Yes") {
                            Card* chainOrDiscard = gameTable->tArea.trade(*itr);
                            if(chainOrDiscard != nullptr) addCardToChain(gameTable->p1, *gameTable, -1, chainOrDiscard);
        ...
        if(playerTurn == 2) { //Player2 turn
            ...
            if(ans == "yes" || ans == "Yes") {
                ...
                while(gameTable->p2.hand.hand.empty() == false) {
                    int position = rand() % (gameTable->p2.hand.hand.size());
            ...
            if(gameTable->tArea.numCards() != 0) {
                for(auto itr = gameTable->tArea.cardTypes.begin(); itr != gameTable->tArea.cardTypes.end(); ++itr) {
                    while(true){
                        ...
                        if(ans == "yes" || ans == "Yes") {
                            Card* chainOrDiscard = gameTable->tArea.trade(*itr);
                            if(chainOrDiscard != nullptr) addCardToChain(gameTable->p2, *gameTable, -1, chainOrDiscard);
                            gameTable->tArea.cardTypes.erase(itr);       
                            break;
                        }
                        if(ans == "no" || ans == "No") break;
